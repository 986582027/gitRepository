...
public class MainCtrl extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if(ac.equals("login"))
		{
			...
			String username = request.getParameter("luname");
			String password = request.getParameter("lupass");
			...
			request.getSession().setAttribute("utype", utype);
				...
				if (list.size() == 1) {
					...
					if (ulist.size() == 1&& password.equals(map.get("upass").toString())) {
							...
							gor("/petsns/index.jsp", request, response);
		...
		if(ac.equals("uploadimg5"))
		{
			...
			go("/js/uploadimg5.jsp?filename="+filename, request, response);
              ...
              String sql = "insert into ptrans(title,itype,tel,qq,savetime,filename,content,uname "+
              ")values('"+title+"','"+itype+"','"+tel+"','"+qq+"','"+savetime+"','"+filename+"','"+content+"','"+uname+"')";
              dao.commOper(sql);
              request.getSession().setAttribute("suc", "suc");
              response.sendRedirect("/petsns/admin/ptranstj.jsp");
              }
              //ptranstj.jsp请求处理结束

              //ptransxg.jsp请求处理开始
              if(ac.equals("ptransxg")){
              ...
              String filename = request.getParameter("filename")==null?"":request.getParameter("filename");
              String content = request.getParameter("content")==null?"":request.getParameter("content");
              String sql = "update ptrans set title='"+title+"',itype='"+itype+"',tel='"+tel+"',qq='"+qq+"',filename='"+filename+"',content='"+content+"' where id="+id ;
              dao.commOper(sql);
              request.getSession().setAttribute("suc", "suc");
              response.sendRedirect("/petsns/admin/ptransxg.jsp?id="+id+"");
              }
              //ptransxg.jsp请求处理结束
